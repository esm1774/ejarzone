...
-- تغيير اسم العمود
ALTER TABLE contracts CHANGE COLUMN next_due_date next_payment_date DATE NOT NULL;

-- إنشاء Trigger الجديد لتحديث تاريخ الاستحقاق القادم بعد كل دفعة
DELIMITER //
CREATE TRIGGER update_next_payment_date AFTER INSERT ON payments
FOR EACH ROW
BEGIN
