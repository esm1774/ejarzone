...
try {
    $query = "
        SELECT 
            ...
            CONCAT('إيجار شقة ', u.unit_name, ' إلى تاريخ ', DATE_FORMAT(p.next_due_date, '%Y/%m/%d')) as payment_description
        FROM 
            payments p
    ...
    // تعيين headers لتحميل الملف
    header('Content-Type: text/html; charset=utf-8');
    header('Content-Disposition: attachment; filename="سند_قبض_' . $payment['receipt_number'] . '.html"');
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .company-info-arabic, .company-info {
            flex: 1;
        }
        .company-name {
            font-size: 24px;
