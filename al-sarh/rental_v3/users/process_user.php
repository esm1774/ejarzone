...
try {
    switch ($action) {
        case 'add':
            ...
            $requiredFields = ['username', 'full_name', 'email', 'password', 'role_id'];
            ...
            $stmt = $db->prepare("SELECT COUNT(*) FROM roles WHERE role_id = ?");
            ...
            $stmt = $db->prepare("INSERT INTO users (username, full_name, email, password, role_id, created_at) 
                                 VALUES (?, ?, ?, ?, ?, NOW())");
            $stmt->execute([
                $_POST['username'],
                $_POST['full_name'],
        ...
        case 'edit':
            ...
            // التحقق من البيانات المطلوبة
            $requiredFields = ['username', 'full_name', 'email', 'role_id'];
            foreach ($requiredFields as $field) {
                if (empty($_POST[$field])) {
                    throw new Exception("جميع الحقول مطلوبة");
                }
