...
try {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'delete') {
        if (!isset($_POST['expense_id']) || !is_numeric($_POST['expense_id'])) {
            echo json_encode(['success' => false, 'message' => 'معرف المصروف غير صحيح']);
            exit();
        }
        ...
        $check = $db->prepare("SELECT expense_id FROM expenses WHERE expense_id = ?");
        $check->execute([$expense_id]);
        if (!$check->fetch()) {
            echo json_encode(['success' => false, 'message' => 'المصروف غير موجود']);
            exit();
        }

        // حذف المصروف
        $stmt = $db->prepare("DELETE FROM expenses WHERE expense_id = ?");
        $success = $stmt->execute([$expense_id]);
        
        if ($success) {
            echo json_encode(['success' => true, 'message' => 'تم حذف المصروف بنجاح']);
        } else {
            echo json_encode(['success' => false, 'message' => 'فشل في حذف المصروف']);
        }
        exit();
    }
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'edit') {
        ...
        if (!empty($_POST['payment_method'])) {
            $check_method = $db->prepare("SELECT method_id FROM payment_methods WHERE method_name = ?");
            $check_method->execute([$_POST['payment_method']]);
            $method = $check_method->fetch(PDO::FETCH_ASSOC);
            if (!$method) {
                echo json_encode(['success' => false, 'message' => 'طريقة الدفع غير موجودة']);
                exit();
            }
    ...
    // التحقق من وجود طريقة الدفع وتحويلها إلى method_id
    $method_id = null;
    if (!empty($payment_method)) {
        $check_method = $db->prepare("SELECT method_id FROM payment_methods WHERE method_name = ?");
        $check_method->execute([$payment_method]);
        $method = $check_method->fetch(PDO::FETCH_ASSOC);
        if (!$method) {
            echo json_encode(['success' => false, 'message' => 'طريقة الدفع غير موجودة']);
            exit();
    ...
    $query = "INSERT INTO expenses (type_id, amount, expense_date, description, receipt_number, method_id, payee_name) 
             VALUES (:type_id, :amount, :expense_date, :description, :receipt_number, :method_id, :payee_name)";
    $params = [
        'type_id' => $type_id,
        'amount' => $amount,
